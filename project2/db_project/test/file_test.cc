...
TEST(FileInitTest, HandlesInitialization) {
  ...
  std::string pathname = "init_test.txt";  // customize it to your test file
  ...
  EXPECT_EQ(num_pages, SIZE*2560 / SIZE)
...
TEST_F(FileTest, HandlesPageAllocation) {
  ...
  page_t* tmp_a, tmp_f;
  file_read_page(fd, allocated_page,tmp_a);
  
  file_read_page(fd, freed_page,&tmp_f);
...
TEST_F(FileTest, CheckReadWriteOperation) {
  ...
  page_t* tmp_test;
  pagenum_t test_pagenum = file_alloc_page(fd);
  
  file_write_page(fd, test_pagenum, tmp_write);
  file_read_page(fd, test_pagenum,tmp_test);
}
